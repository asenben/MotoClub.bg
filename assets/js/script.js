...
document.querySelector(".toggle-menu").addEventListener("click", async () => {
  ...
  if (menu.classList.contains("open")) {
    await closeMenu(menu, links, delayPerLink, animationDuration); // Затваряне на менюто
  } else {
    openMenu(menu, links, delayPerLink); // Отваряне на менюто
  }
});
...
function closeMenu(menu, links, delayPerLink, animationDuration) {
  return new Promise((resolve) => {
    ...
    // Изчакваме всички връзки да приключат преди затваряне на менюто
    ...
    setTimeout(() => {
      ...
      resolve();
    }, totalDelay); // Изчакваме последната връзка да завърши
  });
}
